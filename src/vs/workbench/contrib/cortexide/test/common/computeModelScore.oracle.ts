...
// GENERATED ORACLE -- do not hand-edit. The ModelRouter.scoreModel body as of the extracting
// commit, mechanically transformed (git HEAD body with only capabilities/realParamSize/
// isVisionCapable/freeTier reads turned into injected inputs). INDEPENDENT reference for the
// differential fuzz in computeModelScore.test.ts; if the extracted pure fn ever diverges from
// the original arithmetic, the fuzz fails.

import { localProviderNames, ProviderName } from '../../common/cortexideSettingsTypes.js';
...
import { ComputeModelScoreInputs } from '../../common/routing/computeModelScore.js';

export function oracleComputeModelScore(inputs: ComputeModelScoreInputs): number {
	const { modelSelection, context, capabilities, isVisionCapable, realParamSize, hasOnlineModels, localFirstAI, routingPolicy, localFirstAISetting, getFreeTierRemaining } = inputs;
