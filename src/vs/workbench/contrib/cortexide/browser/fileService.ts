...
class FilePromptActionService extends Action2 {
	private static readonly CORTEXIDE_COPY_FILE_PROMPT_ID = 'cortexide.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.CORTEXIDE_COPY_FILE_PROMPT_ID,
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await cortexideModelService.getModelSafe(uri)).model?.getLanguageId() || '',
