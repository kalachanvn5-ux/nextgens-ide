...
async function copyServerWorkerMain(outDir: string): Promise<void> {
	...
	const localPath = path.join(import.meta.dirname, 'node_modules', 'vscode-markdown-languageserver', 'dist', 'node', 'workerMain.js');
	const rootPath = path.join(import.meta.dirname, '..', '..', 'node_modules', 'vscode-markdown-languageserver', 'dist', 'node', 'workerMain.js');
